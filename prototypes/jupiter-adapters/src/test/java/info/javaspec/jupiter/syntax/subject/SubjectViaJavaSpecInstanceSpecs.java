...
class SubjectViaJavaSpecInstanceSpecs {
	...
	DynamicNode makeTests() {
		...
		// Positive: This is likely to be a lot more robust, if tests get declared in
		// parallel.
		JavaSpec<List<String>> javaspec = new JavaSpec<>();

		// Negative: Have to call methods all the time (but at least there's only 1
		// instance, instead of one per block).
		return javaspec.describe("List", () -> {
			// Positive: It works, in supplying a fresh instance to each spec.
			// Positive: It can call any subject constructor, as long as all data is known
			...
			// Positive: You can receive the subject as a parameter, instead of having to
