...
final class JavaSpec<S> {
	...
	public DynamicTest it(String behavior, Executable verification) {
		// Positive: The algorithm for collecting fixture lambdas is straightforward
		// enough, using a Deque.
	...
	private static class DynamicNodeList extends LinkedList<DynamicNode> {
		...
		public DynamicTest addTest(String behavior, List<Executable> arrangements, Executable verification,
			List<Executable> cleaners) {
			DynamicTest test = DynamicTest.dynamicTest(behavior, () -> {
				for (Executable arrange : arrangements) {
					arrange.execute();
				}

				// Unknown: Should a failed cleanup cause the spec to fail, even if all the
				// assertions passed?
				// I tend to think so, but it would be nice if the impact were clarified.
				// Future work: What if AssertionErrors thrown from afterEach lambdas caused the
				// spec to fail
				// (sometimes they have odd assertions in them, too), but other failures caused
				// a softer kind of failure?
